...
if(isset($_POST['register'])) {
    // Capture all inputs including names
    ...
    if(empty($username) || empty($first_name) || empty($last_name) || empty($email) || empty($password)) {
        ...
        if($check->num_rows > 0){
            ...
            $sql_user = "INSERT INTO users (username, email, password, role) VALUES ('$username', '$email', '$password', '$role')";
            
            if($conn->query($sql_user) === TRUE) {
                // Get the new User ID
                $new_user_id = $conn->insert_id;
                
                // 2. Insert into CLIENTS table immediately with REAL names
                // We set default values for optional fields like department/phone
                $sql_client = "INSERT INTO clients (first_name, last_name, email, role, department, phone_number, user_id) 
                               VALUES ('$first_name', '$last_name', '$email', '$role', 'Not Assigned', 'N/A', '$new_user_id')";
